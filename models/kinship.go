...
type Kinship struct {
	Id                 int       `orm:"column(kinship_id);auto" description:"编号"`
	StudentId          int       `orm:"column(student_id)" description:"学生序号"`
	UserId             int       `orm:"column(user_id)" description:"用户ID"`
	Type               int8      `orm:"column(type)" description:"类型，1紧急联系人，2监护人"`
	Relation           string    `orm:"column(relation);size(10)" description:"关系"`
	Name               string    `orm:"column(name);size(20)" description:"名字"`
	UnitName           string    `orm:"column(unit_name);size(50)" description:"单位名称"`
	ContactInformation string    `orm:"column(contact_information);size(11)" description:"联系方式"`
